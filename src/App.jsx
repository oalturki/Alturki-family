...
function FanChartModal({ centerId, members, onClose }) {
  ...
  // خلايا الأطراف: امتدادٌ وحجمُ خطٍّ موحّدان لكل حلقة، ولا تخلو خلية من الأسماء
  const groupLayout = useMemo(() => {
    const gs = segs.filter((x) => x.group);
    if (gs.length === 0) return {};
    const arcOf = (x) => (x.a1 - x.a0) * (ringIn(x.depth) + ringW(x.depth) / 2);
    // حجم خط موحّد لكل حلقة: نأخذ ما يناسب أضيق الخلايا تقريبًا
    const fsByD = {};
    gs.forEach((x) => {
      const longest = x.names.reduce((mx, t) => Math.max(mx, t.length), 3);
      const arc = arcOf(x);
      const fit = arc >= 26 ? arc / (longest * 0.62) : 7.5;
      (fsByD[x.depth] = fsByD[x.depth] || []).push(fit);
    });
    Object.keys(fsByD).forEach((d) => {
      const arr = fsByD[d].sort((a, b) => a - b);
      fsByD[d] = Math.max(6, Math.min(9, arr[Math.floor(arr.length * 0.3)] || 7.5));
    });
    // العمق المطلوب لكل حلقة ثم توحيده مع سقفٍ لا يشوّه المخطط
    const needByDepth = {};
    gs.forEach((x) => {
      const fs = fsByD[x.depth];
      const arc = arcOf(x);
      const longest = x.names.reduce((mx, t) => Math.max(mx, t.length), 3);
      let need;
      if (arc >= 26) {
        const maxCh = Math.max(4, Math.floor(arc / (fs * 0.62)));
        need = packNames(x.names, maxCh, 99).length * fs * 1.28 + 10;
      } else {
        const cols = Math.max(1, Math.floor(arc / (fs * 1.2)));
        need = Math.ceil(x.names.length / cols) * (longest * fs * 0.62 + 8);
      }
      needByDepth[x.depth] = Math.max(needByDepth[x.depth] || 0, need);
    });
    const map = {};
    gs.forEach((x) => {
      const fs = fsByD[x.depth];
      const rW = ringW(x.depth);
      const cellW = Math.max(rW, Math.min(needByDepth[x.depth] || rW, rW * 2.2));
      const arc = (x.a1 - x.a0) * (ringIn(x.depth) + cellW / 2);
      if (arc >= 26) {
        const lineH = fs * 1.28;
        const maxLines = Math.max(1, Math.floor((cellW - 8) / lineH));
        const maxCh = Math.max(4, Math.floor(arc / (fs * 0.62)));
        map[x.id] = { mode: "lines", fs, cellW, lineH, lines: packNames(x.names, maxCh, maxLines) };
      } else {
        const longest = x.names.reduce((mx, t) => Math.max(mx, t.length), 3);
        const colH = Math.min(cellW, longest * fs * 0.62 + 8);
        const cols = Math.max(1, Math.floor(arc / (fs * 1.2)));
        const rows = Math.max(1, Math.floor(cellW / colH));
        const maxChR = Math.max(3, Math.floor((colH - 6) / (fs * 0.62)));
        map[x.id] = { mode: "cols", fs, cellW, colH, cols, shown: packNames(x.names, maxChR, rows * cols) };
      }
    });
    return map;
  }, [segs, countByDepth]);
...
// تاجٌ مستدير كالشجرة الأصلية: رأس الفرع في القلب، وذرّيته تنتشر حوله في كل الجهات،
// لكل فرعٍ قطاعٌ بقدر كتلته فلا يعبره غصنُ غيره. وتحت القلب سلسلة النسب إلى تركي.
function buildBranchLayout(headId, byId, childrenMap, maxGen) {
  let hidden = 0;
  const FS = (d) => (d === 0 ? 12.5 : d === 1 ? 11 : d === 2 ? 10 : d === 3 ? 9.5 : 9);
  const radiusFor = (name, d) => {
    const fs = FS(d);
    const w = (name || "").length * fs * 0.52;
    return Math.max(d === 0 ? 30 : d === 1 ? 22 : 18, w / 2 + 6);
  };
  const PAD = 10;              // فراغ بين الأسماء المتجاورة على الحلقة
  const RING_MIN = 74;         // أقل مسافة بين حلقتين
  const GAP_A = 0.42;          // فتحة أسفل التاج ينزل منها خطّ النسب
  ...
  // وزن الفرع = عدد أطرافه (لا عدد أفراده) فتتوزّع الزوايا بعدل ولا يبتلع فرعٌ جاره
  const wCache = {};
  const leavesOf = (id, d) => {
    const key = id + "|" + d;
  ...
  // اتساع كل حلقة: يكفي مجموع أقطار أسمائها، فلا تتلاصق ولا تتباعد بلا داعٍ
  const perDepth = {};
  const scan = (id, d) => {
    const m = byId[id] || {};
    if (d > 0) perDepth[d] = (perDepth[d] || 0) + 2 * radiusFor(m.name, d) + PAD;
    const kids = kidsOf(id, d);
    if (kids.length === 0) hidden += sonsOf(id, childrenMap, byId).length;
    kids.forEach((k) => scan(k.id, d + 1));
  };
  scan(headId, 0);
  const maxD = Math.max(1, ...Object.keys(perDepth).map(Number));
  const R = [0, 0];
  for (let d = 1; d <= maxD; d++) {
    const need = (perDepth[d] || 0) * 1.12 / ARC;
    R[d] = Math.max((d === 1 ? 118 : R[d - 1] + RING_MIN), need);
  }

  // إزاحة شعاعية خفيفة تكسر انتظام الحلقة فتبدو الشجرة طبيعية لا هندسية
  const jitter = (id) => {
    let h = 0;
    for (let i = 0; i < id.length; i++) h = (h * 31 + id.charCodeAt(i)) % 1000;
    return (h / 1000 - 0.5) * 0.34 * RING_MIN;
  };

  const nodes = [], links = [];
  const place = (id, d, a0, a1, color, parentIdx) => {
    const m = byId[id] || {};
    const mid = (a0 + a1) / 2;
    const rad = R[d] + (d >= 2 ? jitter(id) : 0);
    const idx = nodes.length;
    nodes.push({
      id, name: m.name || "", depth: d, color,
      fill: d === 0 ? null : mixToWhite(color, Math.min(0.55, (d - 1) * 0.12)),
      alive: m.isAlive !== false, r: radiusFor(m.name, d), fs: FS(d),
      x: Math.cos(mid) * rad, y: Math.sin(mid) * rad,
    });
    ...
    kids.forEach((k, i) => {
      ...
      place(k.id, d + 1, acc, acc + share, d === 0 ? POSTER_COLORS[i % POSTER_COLORS.length] : color, idx);
      acc += share;
    });
    return idx;
  };
  ...
  nodes.push({
    ...
    alive: headM.isAlive !== false, r: radiusFor(headM.name, 0), fs: FS(0),
    x: 0, y: 0, isHead: true,
  });
  const kids0 = sonsOf(headId, childrenMap, byId);
  const w0 = kids0.map((k) => leavesOf(k.id, 1));
  const sum0 = w0.reduce((a, b) => a + b, 0) || 1;
  let acc = A0;
  kids0.forEach((k, i) => {
    const share = ARC * (w0[i] / sum0);
    place(k.id, 1, acc, acc + share, POSTER_COLORS[i % POSTER_COLORS.length], headIdx);
    acc += share;
  });

  // سلسلة النسب أسفل التاج: تبدأ من خارج آخر حلقة حتى لا تعبر الأسماء
  const stemStart = R[maxD] + 60;
  const lineNodes = ancestors.map((a, i) => ({
    id: a.id, name: a.name, y: stemStart + i * 84,
  ...
  const botY = lineNodes.length ? lineNodes[lineNodes.length - 1].y + lineNodes[lineNodes.length - 1].r : Math.max(...ys);
  return {
    nodes, links, lineNodes, stemStart,
    lineage: chain.map((m) => m.name).join(" بن "),
    minX: Math.min(-140, ...xs) - 40, maxX: Math.max(140, ...xs) + 40,
    minY: Math.min(...ys) - 30,
    maxY: botY + 40,
...
function PosterTreeModal({ members, meId, onClose }) {
  ...
  const [saving, setSaving] = useState(false);
  const svgRef = useRef(null);
  useEffect(() => { if (!rootId && branchHeads.length) setRootId(branchHeads[0].m.id); }, [branchHeads]);

  const L = useMemo(() => (rootId ? buildBranchLayout(rootId, byId, childrenMap, maxGen) : null), [rootId, byId, childrenMap, maxGen]);
  const head = rootId ? byId[rootId] : null;
  ...
  const exportPdf = async () => {
    ...
    try {
      const xml = new XMLSerializer().serializeToString(svg);
      const url = "data:image/svg+xml;charset=utf-8;base64," + btoa(unescape(encodeURIComponent(xml)));
      const img = new Image();
      await new Promise((res, rej) => { img.onload = res; img.onerror = rej; img.src = url; });
      const scale = Math.min(2.4, 2600 / W);
      ...
      const j = await ensureJsPdf();
      const pdf = new j.jsPDF({ orientation: W > H ? "l" : "p", unit: "pt", format: [W, H] });
      pdf.addImage(png, "PNG", 0, 0, W, H);
      pdf.save(`شجرة_${head.name}.pdf`);
    } catch (e) { console.error(e); }
    setSaving(false);
  };

  const fsFor = (d) => (d === 0 ? 12 : d === 1 ? 10.5 : d === 2 ? 9.5 : 9);
  const label = (nm, r, fs) => {
    const maxCh = Math.max(3, Math.floor((r * 2) / (fs * 0.55)));
    return nm.length > maxCh ? nm.slice(0, maxCh - 1) + "…" : nm;
  };
    ...
    <div>
      <div style={{ background: T.card, border: `1px solid ${T.line}`, borderRadius: 14, padding: 12, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {[[3, "٣ أجيال"], [4, "٤ أجيال"], [6, "٦ أجيال"], [99, "الكل"]].map(([g, lbl]) => (
            <button key={g} onClick={() => setMaxGen(g)} style={{ flex: 1, background: maxGen === g ? T.sandDark : T.card, color: T.ink, border: `1.5px solid ${maxGen === g ? T.gold : T.line}`, borderRadius: 10, padding: "8px 4px", fontSize: 11.5, fontWeight: 700, fontFamily: "inherit", cursor: "pointer" }}>{lbl}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ overflow: "auto", border: `1px solid ${T.line}`, borderRadius: 12, background: "#F6FAF6", maxHeight: "62vh" }}>
        <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
          <rect x={0} y={0} width={W} height={H} fill="#F6FAF6" />
          <text x={W / 2} y={36} textAnchor="middle" fontSize={18} fontWeight={800} fill="#4A3A22" style={{ fontFamily: "'Aref Ruqaa', serif" }}>
            شجرة {head.name}
          </text>
          <text x={W / 2} y={60} textAnchor="middle" fontSize={11.5} fill="#6B7370" style={{ fontFamily: "'Readex Pro', sans-serif" }}>
            {L.lineage}
          </text>
          ...
          <g transform={`translate(${ox}, ${oy})`}>
            ...
            {L.lineNodes.length > 0 && (
              <path d={`M 0 ${L.stemStart - 48} L 0 ${L.lineNodes[L.lineNodes.length - 1].y}`} stroke="#A98A63" strokeWidth={9} strokeLinecap="round" />
            )}
            {L.lineNodes.map((t) => (
              <g key={"ln" + t.id}>
            ...
            {L.links.map((l, i) => {
              ...
              const k = 0.35;
              return <path key={"l" + i} d={`M ${f.x} ${f.y} Q ${mx + (t.y - f.y) * k * 0.18} ${my - (t.x - f.x) * k * 0.18}, ${t.x} ${t.y}`}
                stroke="#9C8465" strokeWidth={Math.max(1.3, 5.2 - t.depth * 0.85)} fill="none" strokeLinecap="round" opacity={0.92} />;
            })}

            {/* الأفراد */}
            {L.nodes.map((n) => (
              <g key={n.id}>
                <circle cx={n.x} cy={n.y} r={n.r} fill={n.isHead ? "#F0D590" : (n.fill || n.color || "#8FBF8F")} stroke={n.alive ? "#5E3F27" : "#A24936"} strokeWidth={n.isHead ? 2.2 : 1.2} strokeDasharray={n.alive ? "" : "3 2"} />
...
function TreeTab({ members, setMembers, profilesMap, canManageTree, meId, gameInvite, onGameInviteUsed }) {
  ...
  return (
    <div>
      ...
      {posterOpen && (
        <PosterTreeModal members={members} meId={meId} onClose={() => setPosterOpen(false)} />
      )}
    </div>
  );
}
